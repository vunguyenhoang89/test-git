...
{
    $meta_key = '__sparkxlab_meta_img_url';
    $meta_img_url = get_post_meta( $post_ID, $meta_key, true );
    $main_image = get_field( 'the_post_main_image', $post_ID, true );

    if ( $main_image ){
        $main_image = str_replace('post.','',$main_image);
        $meta_img_url = $main_image;
    }

    if( $meta_img_url!="" ){
        $attachmenturl = $meta_img_url;
    }
    else{
        $thumb = wp_get_attachment_image_src(get_post_thumbnail_id($post_ID), 'full');
        if (!empty($thumb)) {
            if (is_array($thumb)) {
                $attachmenturl = $thumb[0];
            } else {
                $attachmenturl = $thumb;
            }
        } else {
            $attachmenturl = get_bloginfo('template_directory')."/common/img/logo.png";
        }
        $attachmenturl = replace_relative_path($attachmenturl);
    }

    $home_url = get_home_url();
    $home_url = rtrim($home_url,'/');
    $home_url = str_replace('post.','',$home_url);
    if(strpos($attachmenturl,$home_url)===false) {
        return $attachmenturl = $home_url . $attachmenturl;
    }
    else{
...
if (!function_exists('replace_relative_path')){
    function replace_relative_path($str)
    {
        global $blog_id;
        if(is_multisite()){
