...
function displaycheck()
{
	if(document.getElementById("xyz_smap_post_permission"))
	{
		...
		if(fcheckid==1)
		{
			...
			document.getElementById("fpmfdateofposting").style.display='';
                        node = document.querySelectorAll("#fpmfdateofposting input[type=text]");
                        node[0].required = true;
		}
		else
		{
			...
			document.getElementById("fpmftarea").style.display='';	
			document.getElementById("fpmfdateofposting").style.display='none';
                        node = document.querySelectorAll("#fpmfdateofposting input[type=text]");
...
//			document.getElementById("twmftarea").style.display='';
			...
			document.getElementById("twaidateofposting").style.display='';
                        node = document.querySelectorAll("#twaidateofposting input[type=text]");
                        node[0].required = true;
		}
		else
		{
...
//			document.getElementById("twmftarea").style.display='none';
			...
			document.getElementById("twaidateofposting").style.display='none';
                        node = document.querySelectorAll("#twaidateofposting input[type=text]");
                        node[0].required = false;
		}
	}
...
{
	var cat_list= ""; var chkdArray = []; var cat_list_array = [];
...
<?php
$sparkxlab_social_auto_publish = get_post_meta($postid, 'sparkxlab_social_auto_publish', true);
?>
...
<tr><td colspan="2" valign="top">&nbsp;</td></tr>
	...
	<tr valign="top">
		<td class="xyz_smap_pleft15" width="50%"><?php _e('Enable auto publish post to my facebook account', 'social-media-auto-publish')?>
		</td>
		<td width="50%">
            <select id="xyz_smap_post_permission" name="xyz_smap_post_permission"
			onchange="displaycheck()">
                <option value="0">No</option>
				<option value="1"<?php if (isset($sparkxlab_social_auto_publish['xyz_smap_post_permission']) && $sparkxlab_social_auto_publish['xyz_smap_post_permission'] == 1) echo ' selected="selected"'?>>Yes</option>
		</select>
		</td>
	</tr>
	<!--<tr valign="top" id="fpmd">
		...
		<td><select id="xyz_smap_po_method" name="xyz_smap_po_method">
				...
				<optgroup label="Text message with attached link">
					<option value="1"
					<?php /* if(get_option('xyz_smap_po_method')==1) echo 'selected="selected"';*/?>>Attach
						your blog post</option>
					<option value="2"
					<?php /* if(get_option('xyz_smap_po_method')==2) echo 'selected="selected"';*/?>>
	...
	<tr id="fpmftarea">
        <td class="xyz_smap_pleft15"><?php _e('Meta Description', 'social-media-auto-publish')?></td>
        <td>
            <?php $xyz_smap_message = isset($sparkxlab_social_auto_publish['xyz_smap_message']) ? $sparkxlab_social_auto_publish['xyz_smap_message'] : ''; ?>
		    <textarea id="xyz_smap_message"  name="xyz_smap_message" style="height:80px !important;" ><?php echo esc_textarea($xyz_smap_message);?></textarea>
	    </td>
    </tr>
    <tr id="fpmfdateofposting">
        <td class="xyz_smap_pleft15"><?php _e('Date Of Posting', 'social-media-auto-publish')?></td>
        <td>
            <div style="float: left;" class="acf-date_picker" data-save_format="yymmdd" data-display_format="yy/mm/dd">
                <input type="hidden" value="<?php echo $sparkxlab_social_auto_publish['xyz_smap_fb_date'];?>" name="xyz_smap_fb_date" class="input-alt" />
                <input type="text" value="" class="input"  />
            </div>
            <div style="float: left; padding-left: 5px; width: 70px;">
                <label><?php _e('H', 'social-media-auto-publish');?>:</label>
                <?php $hours = range(0, 23); ?>
                <select name="xyz_smap_fb_hour">
                    <?php foreach($hours as $h):?>
                        <option<?php  if(isset($sparkxlab_social_auto_publish['xyz_smap_fb_hour']) && $sparkxlab_social_auto_publish['xyz_smap_fb_hour'] == $h) echo ' selected="selected"';?> value="<?php echo $h;?>"><?php echo ($h == 0) ? '00' : ($h < 10) ? '0'.$h : $h;?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div style="float: left; padding-left: 5px; width: 70px;">
                <label><?php _e('M', 'social-media-auto-publish');?>:</label>
                <select name="xyz_smap_fb_minute">
                    <option value = "00" <?php  if(isset($sparkxlab_social_auto_publish['xyz_smap_fb_minute']) && $sparkxlab_social_auto_publish['xyz_smap_fb_minute'] == '00') echo ' selected="selected"';?>>00</option>
                    <option value = "30" <?php  if(isset($sparkxlab_social_auto_publish['xyz_smap_fb_minute']) && $sparkxlab_social_auto_publish['xyz_smap_fb_minute'] == '30') echo ' selected="selected"';?>>30</option>
                </select>
            </div>
        </td>
    </tr>
...
<tr><td colspan="2" valign="top">&nbsp;</td></tr>
	...
	<tr valign="top">
		<td class="xyz_smap_pleft15" width="50%"><?php _e('Enable auto publish posts to my twitter account', 'social-media-auto-publish')?>
		</td>
		<td width="50%"><select id="xyz_smap_twpost_permission" name="xyz_smap_twpost_permission"
			onchange="displaycheck()">
				<option value="0">No</option>
				<option value="1"<?php if (isset($sparkxlab_social_auto_publish['xyz_smap_twpost_permission']) && $sparkxlab_social_auto_publish['xyz_smap_twpost_permission'] == 1) echo ' selected="selected"'?>>Yes</option>
		</select>
		</td>
	</tr>
	...
	<select name="xyz_smap_tw_info" id="xyz_smap_tw_info" onchange="xyz_smap_tw_info_insert(this)">
        ...
        <td class="xyz_smap_pleft15"><?php _e('Title', 'social-media-auto-publish')?><br />
            ( <?php _e('Limit is 70 characters.', 'social-media-auto-publish')?> )</td>
        <td>
            <?php $xyz_smap_twmessage_title = isset($sparkxlab_social_auto_publish['xyz_smap_twmessage_title']) ? $sparkxlab_social_auto_publish['xyz_smap_twmessage_title'] : ''; ?>
            <textarea id="xyz_smap_twmessage_title"  name="xyz_smap_twmessage_title" placeholder="<?php _e('Limit is 70 characters.', 'social-media-auto-publish')?>" maxlength="70" style="height:80px !important;" ><?php echo esc_textarea($xyz_smap_twmessage_title);?></textarea>
        </td>
    </tr>
    <tr id="twmf">
        <td class="xyz_smap_pleft15"><?php _e('Hashtag', 'social-media-auto-publish')?><br />
            ( <?php _e('Limit is 30 characters.', 'social-media-auto-publish')?> )</td>
        <td>
            <?php $xyz_smap_twmessage = isset($sparkxlab_social_auto_publish['xyz_smap_twmessage']) ? $sparkxlab_social_auto_publish['xyz_smap_twmessage'] : ''; ?>
            <textarea id="xyz_smap_twmessage"  name="xyz_smap_twmessage" placeholder="<?php _e('Limit is 30 characters.', 'social-media-auto-publish')?>" maxlength="30" style="height:80px !important;" ><?php echo esc_textarea($xyz_smap_twmessage);?></textarea>
	    </td>
    </tr>
    <tr id="twaidateofposting">
        <td class="xyz_smap_pleft15"><?php _e('Date Of Posting', 'social-media-auto-publish')?></td>
        <td>
            <div style="float: left;" class="acf-date_picker" data-save_format="yymmdd" data-display_format="yy/mm/dd">
                <input type="hidden" value="<?php echo $sparkxlab_social_auto_publish['xyz_smap_tw_date'];?>" name="xyz_smap_tw_date" class="input-alt" />
                <input type="text" value="" class="input"  />
            </div>
            <div style="float: left; padding-left: 5px; width: 70px;">
                <label><?php _e('H', 'social-media-auto-publish');?>:</label>
                <?php $hours = range(0, 23); ?>
                <select name="xyz_smap_tw_hour">
                    <?php foreach($hours as $h):?>
                        <option<?php  if(isset($sparkxlab_social_auto_publish['xyz_smap_tw_hour']) && $sparkxlab_social_auto_publish['xyz_smap_tw_hour'] == $h) echo ' selected="selected"';?> value="<?php echo $h;?>"><?php echo ($h == 0) ? '00' : ($h < 10) ? '0'.$h : $h;?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div style="float: left; padding-left: 5px; width: 70px;">
                <label><?php _e('M', 'social-media-auto-publish');?>:</label>
                <select name="xyz_smap_tw_minute">
                    <option value = "00" <?php  if(isset($sparkxlab_social_auto_publish['xyz_smap_tw_minute']) && $sparkxlab_social_auto_publish['xyz_smap_tw_minute'] == '00') echo ' selected="selected"';?>>00</option>
                    <option value = "30" <?php  if(isset($sparkxlab_social_auto_publish['xyz_smap_tw_minute']) && $sparkxlab_social_auto_publish['xyz_smap_tw_minute'] == '30') echo ' selected="selected"';?>>30</option>
                </select>
            </div>
        </td>
    </tr>
...
<script type="text/javascript">
	...
	function load_edit_action()
	{
		...
		//FB 
				if(document.getElementById("xyz_smap_post_permission"))
				{
					if (document.getElementById("xyz_smap_post_permission").value == 0) {
                        /*document.getElementById("fpmd").style.display='none';
                        document.getElementById("fpmf").style.display='none';*/
//                        document.getElementById("fpmftarea").style.display='none';
        ...
        //TW 
				if(document.getElementById("xyz_smap_twpost_permission"))
				{
                    if(document.getElementById("xyz_smap_twpost_permission").value == 0) {
                        document.getElementById("twai").style.display='none';
                    }
//					document.getElementById("twmf").style.display='none';
//					document.getElementById("twai").style.display='none';
//					document.getElementById("twmftarea").style.display='none';
		...
		//LN								
				if(document.getElementById("xyz_smap_lnpost_permission"))
				{
					if (document.getElementById("xyz_smap_lnpost_permission").value==0) {
                        document.getElementById("lnimg").style.display='none';
                        document.getElementById("lnmf").style.display='none';
                        document.getElementById("shareprivate").style.display='none';
                        document.getElementById("lnmftarea").style.display='none';
